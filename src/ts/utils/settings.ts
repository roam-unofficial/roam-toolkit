...
export const Settings = {
    ...
    isActive: async (featureId: string) =>
        (await getStateFromStorage())[featureId]?.active,
};
    ...
    return features.map((feature: any) => {
        ...
        feature.settings =
            feature?.settings.map((setting: Setting) => {
                ...
                reducers[`${feature.id}_${setting.id}`] = (
                    state: any,
                    action: any
                ) => {
...
const notifySettingsUpdated = () =>
    Browser.sendMessageToActiveTab('settings-updated');
