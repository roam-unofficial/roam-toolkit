...
export const Roam = {
  ...
  getActiveRoamNode(): RoamNode | null {
    ...
    return new RoamNode(
      element.value,
      new Selection(element.selectionStart, element.selectionEnd)
    );
  },
  ...
  async createBlockAtBottom() {
    console.log("creating block at bottom");
    await this.editBlock(getLastTopLevelBlock());
    const node = this.getActiveRoamNode();
    console.log("node", JSON.stringify(node?.text));
    // const tlb = getLastTopLevelBlock();
    // await Mouse.leftClick(tlb,20);
    if (node?.text !== "") {
      console.log("sibling below");
      await this.createSiblingBelow();
    }
  }
};
