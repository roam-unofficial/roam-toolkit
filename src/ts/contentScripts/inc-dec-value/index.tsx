...
const cursorPlacedBetweenBrackets = (text: string, cursor: number): boolean =>
    openBracketsLeftIndex(text, cursor) <
        closingBracketsRightIndex(text, cursor) &&
    closingBracketsLeftIndex(text, cursor) <
        openBracketsLeftIndex(text, cursor);

const cursorPlacedOnNumber = (text: any, cursor: number): boolean =>
    text.substring(0, cursor).match(/[0-9]*$/)[0] +
        text.substring(cursor).match(/^[0-9]*/)[0] !==
    '';

const cursorPlacedOnDate = (text: string, cursor: number): boolean =>
    cursorPlacedBetweenBrackets(text, cursor) &&
    nameIsDate(nameInsideBrackets(text, cursor));

const nameInsideBrackets = (text: string, cursor: number): string =>
    text.substring(
        text.substring(0, cursor).lastIndexOf('[['),
        cursor + text.substring(cursor).indexOf(']]') + 2
    );
...
export const modify = (modifier: (input: number) => number) => {
    ...
    if (cursorPlacedOnDate(node.text, cursor)) {
        ...
        newValue =
            ...
            RoamDate.formatPage(
                modifyDate(
                    dateFromPageName(nameInsideBrackets(node.text, cursor)),
                    modifier
                )
            ) +
            node.text.substring(
                closingBracketsRightIndex(node.text, cursor) + 2
            );
        ...
        const numberStartedAt = node.text.substring(0, cursor)?.match(/[0-9]*$/)
            ?.index!;
        ...
        newValue = node.text.replace(
            datesInContent[0],
            RoamDate.formatPage(
                modifyDate(dateFromPageName(datesInContent[0]), modifier)
            )
        );
    }
