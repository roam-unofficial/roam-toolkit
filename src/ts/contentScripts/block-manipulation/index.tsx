...
export const config: Feature = {
    ...
    settings: [
        ...
        } as Shortcut,
        {
            ...
            onPress: () =>
                navigator.clipboard.writeText(getCurrentBlockReference()),
        } as Shortcut,
        {
            type: 'shortcut',
            id: 'copyBlockEmbed',
            ...
            onPress: () =>
                navigator.clipboard.writeText(getCurrentBlockEmbed()),
        } as Shortcut,
        {
            type: 'shortcut',
            id: 'goToTodayPage',
...
const goToTodayPage = async () => {
    ...
    return Browser.goToPage(
        Roam.baseUrl().toString() + '/' + datePage[':block/uid']
    );
};
...
const duplicate = () => {
    ...
    if (node && selectedText) {
        const newText =
            node.textBeforeSelection() +
            selectedText +
            selectedText +
            node.textAfterSelection();
        Roam.save(
            new RoamNode(
                newText,
                new Selection(
                    node.selection.end,
                    node.selection.end + selectedText.length
                )
            )
        );
    } else {
