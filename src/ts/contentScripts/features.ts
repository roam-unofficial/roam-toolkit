...
export const Features = {
  ...
  async getCurrentKeyMap() {
    const features = (await Features.getActiveFeatures()).filter(
      (it) => it.settings
    );
    const allShortcuts = (await mapAsync(features, this.getKeyMapFor))
      .flat()
      .filter((it) => it[1]);
  ...
  async getKeyMapFor(feature: Feature) {
    return mapAsync(feature.settings!, async (it) => [
      it.id,
      await Settings.get(feature.id, it.id),
    ]);
  },
};
